...
<style>
    ...
    .deskripsi-truncate {
        ...
        max-height: calc(1.4em * 5); /* Pastikan tinggi sesuai jumlah baris */
    }
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active text-muted" aria-current="page">
                    <i class="bi bi-handshake me-1"></i> Kerja Sama
                </li>
            </ol>
        </nav>
    </div>
        ...
        <div class="col-md-8 mb-5">
            <div class="bg-white p-4 rounded-4 shadow-sm">
                <h4 class="fw-bold mb-4 text-dark border-bottom pb-3">
                    <i class="bi bi-handshake-fill me-2 text-primary"></i>Kerja Sama PIK-R
                </h4>

                <div class="row">
                    <?php
                    $query = mysqli_query($conn, "SELECT * FROM kerja_sama WHERE status = 'aktif' ORDER BY urutan ASC, tanggal_input DESC");
                    if (mysqli_num_rows($query) > 0):
                        while ($row = mysqli_fetch_assoc($query)):
                            
                            // [BARU] Siapkan variabel untuk link share
                            $share_id = 'kerjasama-' . $row['id'];
                            $share_url = $page_url_base . '#' . $share_id;
                            $share_title = htmlspecialchars($row['nama_instansi']);
                    ...
                    ?>
                            <div class="col-md-6 mb-4" data-aos="fade-up" id="<?= $share_id ?>">
                                <div class="card h-100 shadow-sm rounded-4 border-0 card-hover">
                                    <?php if (!empty($row['gambar']) && file_exists('../upload/kerjasama/' . $row['gambar'])): ?>
                                        <img src="../upload/kerjasama/<?= htmlspecialchars($row['gambar']) ?>"
                                             class="card-img-top rounded-top-4"
                                             alt="Gambar Kerja Sama"
                                    ...
                                    <div class="card-body d-flex flex-column">
                                        ...
                                        <p class="text-muted small mb-2">
                                            ...
                                            <?= date('d F Y • H:i', strtotime($row['tanggal_input'])) ?>
                                        </p>
                                        <p class="text-dark small flex-grow-1 deskripsi-truncate" style="white-space: pre-line;">
                                        ...
                                        <div class="mt-3 pt-3 border-top">
                                            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                                                <a href="../detail/detail_kerjasama.php?id=<?= $row['id'] ?>" class="btn-custom-detail">
                    ...
                    <?php endwhile;
                        ...
                        <div class="col-12 text-center text-muted py-5 bg-light rounded-3">
                            <i class="bi bi-exclamation-circle fs-4 me-2"></i> Belum ada kerja sama tercatat.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="mb-4">
                <h6 class="fw-bold text-dark mb-3">
                    <i class="bi bi-award-fill me-1 text-warning"></i> Prestasi Organisasi
                </h6>
                <div class="bg-white p-3 rounded-3 shadow-sm">
                    ...
                    ?>
                            <a href="<?= $base_url ?>detail/detail_prestasi.php?id=<?= $p['id'] ?>" class="sidebar-list-item">
                                <h6 class="fw-semibold text-primary small mb-1 text-truncate">
            ...
            <div>
                <h6 class="fw-bold text-dark mb-3">
                    <i class="bi bi-newspaper me-1 text-primary"></i> Berita Organisasi
                </h6>
                <div class="bg-white p-3 rounded-3 shadow-sm">
                     <?php
                    $sidebar_berita = mysqli_query($conn, "SELECT * FROM berita ORDER BY tanggal_terbit DESC LIMIT 5");
                    if (mysqli_num_rows($sidebar_berita) > 0):
                        while ($b = mysqli_fetch_assoc($sidebar_berita)):
                    ?>
                            <a href="<?= $base_url ?>detail/detail_berita.php?id=<?= $b['id'] ?>" class="sidebar-list-item">
                                <h6 class="fw-semibold text-primary small mb-1 text-truncate">
                                    <?= htmlspecialchars($b['judul']) ?>
                                </h6>
                                <div class="text-muted small" style="font-size: 0.75rem;">
                                    <i class="bi bi-calendar3 me-1"></i> <?= date('d M Y', strtotime($b['tanggal_terbit'])) ?>
                                </div>
                            </a>
                    <?php
                        endwhile;
